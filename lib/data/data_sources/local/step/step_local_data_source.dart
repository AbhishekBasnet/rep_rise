...
class WeeklySteps extends Table {
...
@DriftDatabase(tables: [WeeklySteps])
  ...
  MigrationStrategy get migration => MigrationStrategy(
    onUpgrade: (Migrator m, int from, int to) async {
      ...
      if (from < 3) {
        ...
        await m.deleteTable(weeklySteps.actualTableName);
        await m.createTable(weeklySteps);
      }
    },
...
abstract class StepLocalDataSource {
  Future<List<WeeklyStep>> getCachedSteps();
  Future<void> cacheSteps(List<WeeklyStepModel> steps);
...
class StepLocalDataSourceImpl implements StepLocalDataSource {
  ...
  @override
  Future<List<WeeklyStep>> getCachedSteps() {
    return db.select(db.weeklySteps).get();
  }
  ...
  @override
  Future<void> cacheSteps(List<WeeklyStepModel> steps) async {
    await db.batch((batch) {
      batch.insertAll(
        ...
        steps
            .map(
              (s) =>
                  WeeklyStepsCompanion.insert(date: s.date, steps: s.steps, goal: Value(s.goal), dayName: s.dayName),
            )
            .toList(),
  ...
  @override
  Future<void> deleteAllSteps() {
    return db.delete(db.weeklySteps).go();
  }
}
